...
DIM = 224
X_MIN, X_MAX = (110, 114) # (100, 124) # (90, 134) # (70, 150) # (28, 180)
ONE_QUARTER = (X_MAX - X_MIN) // 3
X_MIN_34, X_MAX_34 = (X_MIN + ONE_QUARTER, X_MAX - ONE_QUARTER)
BUFFER = 10
...
def rand_size_2():
    ...
    return random.randint(SIZE_MIN + 20, SIZE_MAX + 20)
...
class Ellipse(Shape):
    ...
    def draw(self, image):
        ...
        # image.draw.polygon(self.coords, PENS_THICK[self.color])
...
class Rectangle(Shape):
    def init_shape(self, min_skew=1.5):
        ...
        self.dx = SCALER * self.dx
        self.dy = SCALER * self.dy
        half_dx = self.dx // 2
        half_dy = self.dy // 2
        shape = box(self.x - half_dx, self.y - half_dy, self.x + half_dx, self.y + half_dy)
        # Rotation
...
class Square(Rectangle):
    def init_shape(self):
        ...
        self.size = SCALER * rand_size()
        half_size = self.size // 2
        shape = box(self.x - half_size, self.y - half_size, self.x + half_size, self.y + half_size)
        # Rotation
        shape = affinity.rotate(shape, random.randint(90))
        self.shape = shape
