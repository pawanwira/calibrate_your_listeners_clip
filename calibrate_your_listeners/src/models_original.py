...
class Listener(nn.Module): # L_0
    def __init__(self, feat_model, embedding_module, with_embed,
        ...
        self._joint_embed_space = joint_embed_space
        self._is_old = is_old
        # First set COCO tokens
        if self._is_old:
            self.vocab_size = embedding_module.num_embeddings
        else:
            self._tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
        ...
        self.lang_model = RNNEncoder(self.embedding, is_old=is_old) # g
        self.feat_model = feat_model # f_L(I_t)
        self.image_feat_size = feat_model.final_feat_dim

        if self._joint_embed_space:
            self.image2Joint = nn.Linear(self.image_feat_size, self._joint_embed_space, bias=False)
            self.lang2Joint = nn.Linear(self.lang_model.hidden_size, self._joint_embed_space, bias=False)
        else:
    ...
    def forward(self, feats, lang, lang_length, average=False, used_as_internal_listener=False):
        ...
        """
        if average:
            weights = torch.mul(1/((1-lang[:,:,EOS_IDX]).sum(1).unsqueeze(1).repeat(1,MAX_SEQ_LEN)),(1-lang[:,:,EOS_IDX])).unsqueeze(1).repeat(1,3,1)
            scores = 0
            for i in range(0,self._max_seq_len-1):
                # Embed features
