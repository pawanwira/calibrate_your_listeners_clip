...
class Speaker(nn.Module): # L_0
    ...
    def init_image_feature_model(self):
        ...
        # if self.config.dataset_params.data_dir == "clip/sw":
        #     self.feat_model = vision_clip.Conv4() # f_L(I_t)
        # else:
        #     self.feat_model = vision.Conv4()
    ...
    # y is targets
    def teacher_forcing_forward_original(self, feats, seq, targets): # length, targets): # y):
        # import pdb; pdb.set_trace()
    ...
    def teacher_forcing_forward(self, feats, targets, seq, activation='gumbel', tau=1.0, length_penalty=False):
        # import pdb; pdb.set_trace()
        ...
        for i in range(max_len):  # Have room for EOS if never sampled
            ...
            # TODO: tokenization penalization
    ...
    def get_tokens_score(self, lang, lang_length, targets, feats_clip):
        import pdb; pdb.set_trace()
        lang_tensor = torch.cat(lang, 1)
        # lang_length += 1  # buffer since set of tokens to be checked might not have EOS token yet
        lang_length = torch.Tensor(lang_length)

        clip_scorer = self.clip_scorer(
                listener=None, # temp placeholder, TODO
                imgs=feats_clip,
                tokenizer = None, # temp placeholder, TODO
                preprocess= None, # temp placeholder, TODO
        ...
        losses = loss(lis_scores, targets)
        return losses
    
    def teacher_forcing_forward_inputnotgt(self, feats, feats_clip, targets, seq, activation='gumbel', tau=1.0, length_penalty=False):
        import pdb; pdb.set_trace()
        ...
        token_prag_loss = torch.Tensor([0])
        ...
        for i in range(max_len):  # Have room for EOS if never sampled
            ...
            token_prag_loss += self.get_tokens_score(lang, lang_length, targets, feats_clip)
        ...
        # if lang_tensor.size(1) != 77:
        #     import pdb; pdb.set_trace()
        return lang_tensor, lang_length, eos_loss, lang4tfloss_tensor, token_prag_loss # , lang_prob
