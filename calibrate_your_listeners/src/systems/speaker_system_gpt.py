...
from transformers import CLIPTokenizer, GPT2Tokenizer
...
class SpeakerCLIPSystem(system.BasicSystem):
    ...
    def load_speaker(self):
        self.model = speaker_gpt.Speaker(config=self.config)
    ...
    def _process_gt_clip(self, gt):
        if isinstance(gt, dict):
            result = []
            for seq_id in range(gt['input_ids'].shape[0]):
                result.append(self.clip_tokenizer.decode(gt['input_ids'][seq_id]))
            return result
    ...
    def get_losses_for_batch(self, batch, batch_idx, which_listener, prefix):
        ...
        outputs = self.model(imgs_clip, utterances)
        logits = outputs.logits[:, self.model.prefix_length - 1: -1]
        # TODO: replace tokens with appropriate data
        gpt_loss = F.cross_entropy(logits.reshape(-1, logits.shape[-1]), tokens.flatten(), ignore_index=0)  
    ...
    def save_lang_table(self, df, batch_idx, prefix):
        ...
        #     vocab = "big_vocab"
        vocab = "clip_vocab"
        exp_name = "clip_pragmatic_loss_batchsize8_lr0-00001" 
        fpath = os.path.join(
